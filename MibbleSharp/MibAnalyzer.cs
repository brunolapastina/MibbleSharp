   ...
   internal class MibAnalyzer : Asn1Analyzer
   {
      ...
      public override Node ExitMacroDefinition(Production node)
      {
         ...
         MibMacroSymbol symbol = new MibMacroSymbol(
             ...
             name)
         {
            Comment = MibAnalyzerUtil.GetComments(node)
         };
      ...
      public override Node ExitTypeAssignment(Production node)
      {
         ...
         // Create type symbol
         if (!(this.GetValue(this.GetChildAt(node, 2), 0) is MibType type))
         {
            throw new ParseException(
                ParseException.ErrorType.Analysis,
                "Expecting MibType",
      ...
      public override Node ExitDefinedType(Production node)
      {
         ...
         if (value is IConstraint constraint)
         {
            value = new TypeReference(loc, local, name, constraint);
         }
      ...
      public override Node ExitValueConstraint(Production node)
      {
         ...
         else
         {
            ...
            {
               if (item is bool boolean && strictLower == false)
               {
                  strictLower = boolean;
               }
               else if (item is bool boolean1)
               {
                  strictUpper = boolean1;
               }
      ...
      public override Node ExitObjectIdentifierValue(Production node)
      {
         ...
         for (int i = 0; i < components.Count; i++)
         {
            ...
            if (number.HasNumber)
            {
               ...
               else if (parent is ObjectIdentifierValue value1)
               {
                  try
                  {
      ...
      public override Node ExitNameOrNumber(Production node)
      {
         ...
         else if (obj is string objStr)
         {
            vref = new ValueReference(
                ...
                this.TopContext,
                objStr);
            value = new NamedNumber(objStr, vref);
         }
         else
         {
      ...
      public override Node ExitNameAndNumber(Production node)
      {
         ...
         if (obj is BigInteger integer)
         {
            value = new NamedNumber(name, integer);
         }
         else
         {
      ...
      public override void ChildSnmpObjectTypeMacroType(Production node, Node child)
      {
         ...
         {
            type = (MibType)this.GetValue(child, 0);
            if (type is IMibContext context)
            {
      ...
      public override void ChildSnmpVariationPart(Production node, Node child)
      {
         ...
         {
            type = (MibType)this.GetValue(child, 0);
            if (type is IMibContext context1)
            {
               this.PushContextExtension(context1);
            }
         }
